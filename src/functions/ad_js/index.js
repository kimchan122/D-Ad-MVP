...
async function calculateProof(w, x) {
  let provider = new providers.Web3Provider(window.ethereum);
  await provider.send("eth_requestAccounts", []);
  let contract = new Contract(privateSoulMinterAddress, PRIVATESOULMINTER_JSON.abi, provider.getSigner());  
  ...
  let wasmBuff = await getFileBuffer(`${DOMAIN}/ad.wasm`);
  let zkeyBuff = await getFileBuffer(`${DOMAIN}/ad_0001.zkey`);

  try {
    let proof = await generateProof(wasmBuff, zkeyBuff, w, x);
  } catch (error) {
...
async function callAds() {
  let provider = new providers.Web3Provider(window.ethereum);
  let contract = new Contract(RegisterAdsContract, abi, provider.getSigner());
